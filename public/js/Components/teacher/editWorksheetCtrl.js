app.controller('editWorksheetCtrl', ['$scope', '$window', function ($scope, $window) {
    ...
    $scope.sheetGroupID = $scope.worksheetData.sheet_group_id;
    $scope.quizzes = $window.quizzes;
    $scope.sharedUser = findSheetSharedUserNotMe($scope.worksheetData.id,$scope.thisUser.id);
    $scope.selectTeacher = [];
    ...
    $(document).ready(function () {
        // Sheet group
        $('#sheet_group').val($scope.worksheetData.sheet_group_id);
        // Checked Old Shared User
        setCheckedOldSharedUser();
    });

    var fileData = readFileSh($scope.worksheetData);
    // Sheet objective
    ...
    $scope.input = '';
    if ($scope.worksheetData.sheet_input_file) {
    ...
    $scope.main = '';
    if ($scope.worksheetData.main_code) {
        $scope.classTestMode = '1';
        $scope.main = fileData.main;
    }

    // Sheet casesensitive
    $scope.casesensitive = $scope.worksheetData.case_sensitive;

    // Sheet score
    $scope.sheetScore = $scope.worksheetData.full_score;
    ...
    $scope.editWorksheet = function () {
        ...
        if ($scope.completeSheetName) {
            if ($scope.sheetName === $scope.worksheetData.sheet_name) {
